...
namespace WFC
{
  bool Utility::LoadBlocksFromDirectory(const FString& DirectoryPath, TArray<FBlockWFC>& OutBlocks, bool AddEmpty, bool AddFill)
  {
		OutBlocks.Empty();
    ...
    UE_LOG(LogTemp, Warning, TEXT("Processed %d/%d Assets."), SuccessCount, StaticMeshCount);
		...
		if (AddEmpty)
		{
			FSocketWFC EmptySocket = FSocketWFC(0, false, false, false, false);
			TArray<FSocketWFC> EmptySockets = { EmptySocket, EmptySocket, EmptySocket, EmptySocket, EmptySocket, EmptySocket };
			FBlockWFC EmptyBlock = FBlockWFC(nullptr, EmptySockets);
		...
		if (AddFill)
		{
			FSocketWFC FillSocket = FSocketWFC(1, false, false, false, false);
			TArray<FSocketWFC> FillSockets = { FillSocket, FillSocket, FillSocket, FillSocket, FillSocket, FillSocket };
			FBlockWFC FillBlock = FBlockWFC(nullptr, FillSockets);
  ...
  bool Utility::CreateBlocks(const FString& AssetName, UStaticMesh* Mesh, TArray<FBlockWFC>& OutBlocks)
  {
    ...
    Sockets.Reserve(6);
      ...
      Socket.Id = FCString::Atoi(*Tokens[i]);
		...
		FBlockWFC NewBlock = FBlockWFC(Mesh, Sockets, 0, Priority);
		...
		{
			FBlockWFC RotatedBlock = RotateBlock(NewBlock, (i * 90));
	...
	FBlockWFC Utility::RotateBlock(const FBlockWFC& Block, const int& Rotation)
	{
		FBlockWFC RotatedBlock = Block;
